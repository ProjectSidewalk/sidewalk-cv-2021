...
for label in {1..4}; do
  ...
  for city in ${cities[@]}; do
    ...
    python ../eval.py ${experiment}_${model_name} ${experiment}_${model_name}_${labels[$label - 1]} $image_base_path $csv_base_path/"tmp/"$city/"test_set_"${labels[$label - 1]}".csv" $model_name $model_save_folder/$experiment $visualizations_path/$experiment/$city $crop_size
    # analyze results
